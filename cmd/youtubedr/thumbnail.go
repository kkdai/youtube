...
var thumbnailCmd = &cobra.Command{
	...
	Example: `youtubedr thumbnail -x 720 -o thumbnail.png https://www.youtube.com/watch\?v\=TGqoAUaivOY`,
...
func init() {
	...
	thumbnailCmd.Flags().UintVarP(&minRes, "min-resolution", "n", 0, "The minimum resolution.")
	thumbnailCmd.Flags().UintVarP(&maxRes, "max-resolution", "x", math.MaxUint, "The maximum resolution.")
	thumbnailCmd.Flags().BoolVarP(&noExtend, "known-only", "k", false, "Whether to only try thumbnails received in video response (lower quality).")
}
...
func downloadThumbnail(url string) error {
	...
	thumbnails = thumbnails.MinHeight(minRes)
